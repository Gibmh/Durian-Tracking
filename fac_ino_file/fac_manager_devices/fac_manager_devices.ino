...
const int pumpPins[NUM_PUMPS] = { 14, 12 };  // D5, D6, D7, D8
...
void sendSensorData(const String& idesp) {
  ...
  // Kiểm tra nếu thời gian chưa đủ khoảng interval
  unsigned long currentMillis = millis();
  if (currentMillis - lastSendTime < sendInterval) {
    return; // Không gửi dữ liệu nếu chưa đủ thời gian
  }
  ...
  previousHumidity = humidity; // Cập nhật giá trị độ ẩm trước đó
}
...
void Web() {
  ...
  if (abs(currentHumidity - previousHumidity) >= HUMIDITY_THRESHOLD || 
      (currentMillis - lastSendTime >= SEND_INTERVAL)) {
